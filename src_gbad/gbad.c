           ...
           parameters->minAnomalousScore))
      {
	    ...
	    printf("\n");
         ...
         PrintAnomalousInstance(firstInstanceListNode->instance, posGraph, 
	                        parameters);
//////////////////////////////////////////////////////////// WFE - PLADS //////
         // If PLADS using GBAD, save anomalous graph to file
         if (parameters->plads)
         {
            instanceNum++;
            instanceGraph = InstanceToGraph(firstInstanceListNode->instance, posGraph);
            WriteAnomGraphToFile(instanceGraph, parameters->labelList, 0,
                                 0, instanceGraph->numVertices,
                                 parameters->partitionNum, instanceNum,
                                 firstInstanceListNode->instance->probAnomalousValue);
            WriteAnomInstanceToFile(firstInstanceListNode->instance, posGraph,
...
//////////////////////////////////////////////////////////// WFE - PLADS //////
               ...
               if (parameters->plads)
               {
                  ...
                  WriteAnomGraphToFile(instanceGraph, parameters->labelList, 0, 
                                       ...
                                       parameters->partitionNum, instanceNum,
                                       instanceListNode->instance->infoAnomalousValue);
                  WriteAnomInstanceToFile(instanceListNode->instance, posGraph, 
                                          parameters, instanceNum);
                  FreeGraph(instanceGraph);
               }
//////////////////////////////////////////////////////////// WFE - PLADS //////
...
/////////////////////////////////////////////// WFE - PLADS2 //////////////////////
   ...
   if (parameters->bestSub)
   {
      ...
      while (subListNode != NULL)
      {
         ...
         {
            if (subListNode->sub->definition != NULL)
            {
               // print sub to temp file
               Graph *graph = subListNode->sub->definition;
               LabelList *labelList = parameters->labelList;
               FILE *filePtr = fopen("tempBestSub.g","w");
               ULONG v, e;
               Edge *edge = NULL;
               // write vertices
               for (v = 0; v < subListNode->sub->definition->numVertices; v++)
               {
                  fprintf(filePtr, "v %lu ", (v + 1));
                  WriteLabelToFile(filePtr, graph->vertices[v].label, labelList, FALSE);
                  fprintf(filePtr, "\n");
               }
               // write edges
               ...
               {
                  edge = &graph->edges[e];
                  if ((edge->vertex1 >= 0) && (edge->vertex1 < subListNode->sub->definition->numVertices))
                  {
                     if (edge->directed)
                        fprintf(filePtr, "d");
                     else
                        fprintf(filePtr, "u");
                     fprintf(filePtr, " %lu %lu ",
                        (edge->vertex1 + 1),
                        (edge->vertex2 + 1));
                     WriteLabelToFile(filePtr, edge->label, labelList, FALSE);
                     fprintf(filePtr, "\n");
                  }
               }
               fclose(filePtr);
            ...
            // see if this sub matches best sub
//WFE
//
// NOTE:  I have hardcoded the gm command until I figure out a good way to handle it...
//
//WFE
            int status;
            char gmCommand[128];
            //sprintf(gmCommand, "/data/plads2/gbad-mdl/gm ");
            sprintf(gmCommand, parameters->executablePath);
            strcat(gmCommand,"/gm ");
            strcat(gmCommand,parameters->bestSubFile);
            strcat(gmCommand," tempBestSub.g");
            ...
            if (status == 0)
            {
               ...
               bestSub = bestSubListNode->sub;
               bestNumVertices = bestSubListNode->sub->instances->head->instance->numVertices;
               printf("\nFound user-specified best substructure in list of best substructures.\n");
               break;
            }
            else   // set to first best sub as default
            {
               bestSub = subList->head->sub;
               bestNumVertices = subList->head->sub->instances->head->instance->numVertices;
            }
      ...
      if (bestSubListNode != NULL)
      {
         printf("\nUsing the following substructure as the best substructure:\n\n");
         PrintSub(bestSub,parameters);
         printf("\n");
      }
   }
